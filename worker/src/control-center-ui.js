...
.preview{border:1px dashed #cfcfc7;border-radius:12px;background:#fafaf8;padding:14px;min-height:110px}.previewOnly{font-size:9px;font-weight:750;letter-spacing:.05em;color:#8a5b00;margin-bottom:8px}.adCard{max-width:520px;border:1px solid #deded8;border-radius:12px;background:#fff;padding:12px}.adMedia{display:flex;align-items:center;justify-content:center;background:transparent;border-radius:9px;overflow:hidden;margin:8px 0;max-height:300px}.adMedia img,.adMedia video{display:block;width:100%;height:auto;max-height:300px;object-fit:contain}.label{font-size:9px;text-transform:uppercase;letter-spacing:.08em;color:#777}.headline{font-weight:700;margin-top:5px}.bodyCopy{font-size:11px;line-height:1.5;color:#444;margin-top:5px}.cta{display:inline-block;margin-top:9px;font-size:10px;text-decoration:underline}.disclaimer{font-size:9px;color:#888;margin-top:7px}
  ...
  <div id="app" class="grid hidden">
    ...
    <main class="content">
      ...
      <section data-panel="ads" class="panel hidden">
        ...
        <div class="preview"><div class="previewOnly">PREVIEW ONLY · perubahan belum live sampai tombol Publish ditekan</div><div id="adPreview"></div></div>
...
function renderAdPreview(reportError){
  ...
  if(el("placeInterstitial").checked){
    ...
    var placementLabel=document.createElement("div");placementLabel.className="label";placementLabel.textContent="INTERSTITIAL PREVIEW";second.insertBefore(placementLabel,second.firstChild);var lock=document.createElement("div");lock.className="disclaimer";lock.textContent="Runtime tampil sebagai modal overlay · Random 2–5 seconds · LOCKED";second.appendChild(lock);host.appendChild(second);
  }
  return true;
}
...
function demoImage(width,height,name){
  return new Promise(function(resolve){
    ...
    x.fillStyle="#fff";x.fillRect(0,0,width,height);x.strokeStyle="#1b1b1b";x.lineWidth=Math.max(3,width/300);x.strokeRect(2,2,width-4,height-4);
...
async function demoVideo(){
  ...
  await new Promise(function(resolve){
    function frame(now){var t=(now-start)/1000;x.fillStyle="#fff";x.fillRect(0,0,540,540);x.strokeStyle="#1b1b1b";x.lineWidth=3;x.strokeRect(2,2,536,536);x.fillStyle="#171717";x.textAlign="center";x.font="700 30px system-ui";x.fillText("DEMO SPONSOR",270,220);x.font="600 25px system-ui";x.fillText("BMP Terbuka",270,270);x.font="400 17px system-ui";x.fillText("Materi iklan contoh",270,315);x.fillRect(0,532,Math.min(540,540*t/5),4);if(t<5)requestAnimationFrame(frame);else resolve()}requestAnimationFrame(frame);
  });
...
el("previewAds").onclick=function(){if(renderAdPreview(true))status(el("adsStatus"),"PREVIEW ONLY — belum live. Klik Publish untuk menerapkan ke target channel.",true)};
