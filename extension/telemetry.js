...
async function ensureAnalyticsId(){const data=await chrome.storage.local.get(STORAGE_KEY);const current=clean(data?.[STORAGE_KEY],80);if(/^[0-9a-f-]{36}$/i.test(current))return current;const id=crypto.randomUUID();await chrome.storage.local.set({[STORAGE_KEY]:id});return id;}
